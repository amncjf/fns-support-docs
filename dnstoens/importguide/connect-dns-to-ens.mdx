---
title: "Connect a DNS name to the FNS Protocol"
description: "An Overview of Connecting a DNS name to the FNS Protocol"
sidebar_position: 1
---

Other guides found on this site rely on using Cloudflare as the DNS host provider. Cloudflare offers a free-tier level of DNS Hosting that also comes with an optional IPFS gateway solution. The IPFS gateway feature is used in the [Host in Parallel DNS and FNS content from IPFS](/dnstoens/importguide/host-in-parallel) and [Query Blockchain Content through DNS Records](/dnstoens/importguide/dynamic-blockchain-content) guides.
...
## Step 2: Set a TXT record to your Filecoin Wallet Address

Next, create a DNS TXT record entry. This record will prove domain ownership and allow you to specify which Filecoin wallet address will have Controller permissions of the DNS domain name that will be enabled for blockchain functionality.
...
## Step 3: Connect the DNS domain to the FNS Protocol

After DNSSEC is turned on and the the TXT record is create, your DNS domain will be ready for bridging to the FNS protocol. The [FNS Manager App](https://app.fildomains.com) will indicate the domain is ready to register within FNS.
