---
title: "Mapping of DNS vs FNS"
description: "An overview of Mapping of DNS vs FNS"
last_updated:
  author: zadok7.eth
sidebar_position: 7
---
...
This is possible because the storage of this data is distributed among all of the validator nodes that secure the entire Filecoin Virtual Machine blockchain. The blockchain data is stored in an immutable ledger. This means ones the transactions are added to the blockchain, they cannot be reversed, altered or removed.

Transaction data can be queried by services like  [Etherscan.io](https://etherscan.io). Below is an example of recent transactions found on Etherscan for wallets that have interacted with the [FIL Registrar Controller](https://etherscan.io/txs?a=0x283af0b28c62c092c9727f1ee09c02ca627eb7f5) which is the smart contract responsible for registering FNS names.

<div align="center">
  <BrowserWindow url="https://etherscan.io" img="/img/dnstoens/etherscan-registrations.png"/>
</div>

Data security for the Filecoin blockchain is afforded by network consensus, where multiple nodes must agree on the state of the data. In addition, access to update blockchain data is cryptographically secured.
...
For instance, a user could not be blocked, or worse, have their email address hacked and unauthorized access gained to reconfigure what Filecoin blockchain address their FNS name points to. That can only be accessed through having the private keys of the cryptographic wallet that has permission to update such record.
...
An ERC-721 NFT exists on the Filecoin blockchain and would be able to appear on any market without the need to “add” it to each market. This is because the data (or database) is stored within the global EVM’s blockchain.

## Related

* [.eth Permanent Registrar](https://docs.ens.domains/contract-api-reference/.eth-permanent-registrar)
* [Filecoin.org - Introduction to smart contracts](https://ethereum.org/en/smart-contracts/)
