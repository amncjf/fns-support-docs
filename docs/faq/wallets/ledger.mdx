...
Interacting with smart contracts such as FNS using your Ledger requires `Blind signing` to be enabled on your device.
:::tip instructions
* Start and unlock your Ledger Nano S or X.
* Open the Filecoin App.
* Go to `Settings` **→** `Blind signing` and press both buttons to enable it.
:::
Now you should be able to interact with smart contracts and FNS.
